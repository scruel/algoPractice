    ...
    public void solve(int testNumber, InputReader in, OutputWriter out) {
        ...
        while (T-- > 0) {
            ...
            while (true) {
                ...
                char ch;
                while ((ch = os.charAt(i++)) != ':' && ch != '}') k.append(ch);
                //WA-RE-forgot check this
                if (ch == '}') break;
                while ((ch = os.charAt(i++)) != ',' && ch != '}') v.append(ch);
            ...
            while (true) {
                ...
                StringBuilder v = new StringBuilder();
                char ch;
                while ((ch = ns.charAt(i++)) != ':' && ch != '}') k.append(ch);
                if (ch == '}') break;
                while ((ch = ns.charAt(i++)) != ',' && ch != '}') v.append(ch);
                String ks = k.toString();
                String vs = v.toString();

                if (map.containsKey(ks)) {
                    if (!map.get(ks).equals(vs)) chList.add(ks);
                    map.remove(ks);
                } else {
                    naList.add(ks);
                }
            ...
            if (flag) out.writeln("No changes");
            out.writeln();
        }
    }
}
