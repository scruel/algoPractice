...
public class Task506 {
    ...
    boolean needed(int id) {
        if (dep2Map.get(id) != null) {
            for (int cid : dep2Map.get(id)) {
                if (status.get(cid) != 0) return true;
            }
        }
        return false;
    }
    ...
    public void solve(int testNumber, InputReader in, OutputWriter out) {
        ...
        while ((s = in.nextLine()) != null && !s.isEmpty()) {
            ...
            if (ch == 'D') {
                int fid = ID(ss[1]);
                if (!depMap.containsKey(fid)) { depMap.put(fid, new LinkedList<Integer>());}
                for (int i = 2; i < ss.length; i++) {
                    int cid = ID(ss[i]);
                    if (!dep2Map.containsKey(cid)) dep2Map.put(cid, new LinkedList<Integer>());
