...
public class GetMinStack {
    ...
    public void push(int node) {
        stackData.push(node);
        if (stackMin.isEmpty() || stackMin.peek() >= node) stackMin.push(node);
    }

    public void pop() {
        int temp = stackData.pop();
        if (!stackMin.isEmpty() && stackMin.peek() == temp) stackMin.pop();
    }
    ...
    public int min() {
        ...
        if (!stackMin.isEmpty() && stackMin.peek() <= temp) temp = stackMin.peek();
        return temp;
    }
