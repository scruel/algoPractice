...
public class TaskB {
    ...
    public void solve(int testNumber, InputReader in, OutputWriter out) {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = i + 1; j < m; j++) {
                int r;
                if (hm[i][0] <= 0 || hm[i][0] > n) continue;
                if (hm[j][0] <= 0 || hm[j][0] > n) continue;
                if (hm[i][1] <= 0 || hm[i][1] > hs[hm[i][0] - 1]) continue;
                if (hm[j][1] <= 0 || hm[j][1] > hs[hm[j][0] - 1]) continue;
                int l1 = hm[i][0];
                int l2 = hm[j][0];
                if (l1 > l2) {
                    int tmp = l1;
