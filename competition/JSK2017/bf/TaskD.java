...
//        for (int i = 1; i < n + 10; i++) id[i] = i;
        for (int i = 1; i < n + 10; i++) adj[i] = new EdgeList();
        for (int i = 1; i < n + 10; i++) adjc[i] = new EdgeList();
        for (int i = 1; i < m + 10; i++) city[i] = new CityList();
        for (int i = 1; i < m + 10; i++) cityx[i] = new CityList();
//        adjC = new MyList[n];
...
//            }
        ...
        int t = in.nextInt();
        dijkstra(s, t);
        if (!mk[t] || !mk[s]) out.writeln(-1);
        else out.writeln(distTo[t]);
    }
