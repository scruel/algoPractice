...
public class TaskC {
  ...
  boolean check(double dx, double dy, double scale, double t) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (Math.abs(xp - xpp) < EPS && Math.abs(yp - ypp) < EPS) {
        cnt++;
      }
      if (cnt > n / 2) {
        return true;
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int scale = 1; scale <= 10; scale++) {
        ...
        while (ub - lb > 1 && cnt < 100000) {
          ...
          if (C(nums[i][0], nums[i][1], nums[i][2], nums[i][3], dx, dy, scale, mid) > 0) {
            lb = mid;
          }
          else {
            ub = mid;
  ...
  class Pair implements Comparable<Pair> {
    ...
    public boolean equals(Object obj) {
      Pair tm = (Pair) obj;
      if (Math.abs(this.x - tm.x) >= EPS) {
        return false;
      }
      if (Math.abs(this.y - tm.y) >= EPS) {
        return false;
      }
      if (Math.abs(this.scale - tm.scale) >= EPS) {
        return false;
      }
      if (Math.abs(this.theta - tm.theta) >= EPS) {
        return false;
      }
      return true;
    }
