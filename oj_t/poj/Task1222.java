...
public class Task1222 {
  ...
  public static void main(String[] args) {
    Task1222 task1222 = new Task1222();
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = 0; j < 5; j++) {
        for (int k = 0; k < 6; k++) {
          task1222.puzzle[j][k] = input.nextInt();
        }
      }
      task1222.dfs(0);
      for (int j = 0; j < 5; j++) {
        for (int k = 0; k < 6; k++) {
          System.out.print(task1222.result[j][k] + " ");
        }
        System.out.println();
      }
    }
  ...
  // 局部枚举第一行，剩下的直接按照上一行的状态进行开关，结束后检测是否可用。
  boolean dfs(int pos) {
    if (pos == 6) {
      return ok();
    }
